...
const Film = (props) => {
  ...
  const onRequest = () => {
    ...
    onFilmListLoading();
    newService.getRelated(id).then(onListLoaded);
    getGenresList();
    getFilm(id);
    getReviews(id);
  };
  ...
  function getFilm(id) {
    newService.getFilmByID(id).then(onFilmLoaded);
    checkStorage(id);
  }

  //Get reviews
  function getReviews(id) {
    newService.getReviews(id).then(onReviewsLoaded);
  }
      ...
      <Col>
        ...
        <div className="some">
          <Card bg="light" text="dark" style={{ width: "14rem" }} className="cardList">
            ...
            <Badge bg="danger" className="favor" onClick={() => onToggleFavorite(movie.id, movie)}>
              {!like === true ? "Add" : "Remove"}
            </Badge>
          </Card>
        ...
        {reviews.map((review) => (
          <Card>
            <Card.Body>
              ...
              <Card.Subtitle className="mb-2 text-muted">{review.author_details.name}</Card.Subtitle>
  ...
  let comments = renderComment(reviews);

  return (
    <>
      <Container className="justify-content-md-center">
        <Col>{loading ? <Spinner /> : <SelectedMovie movie={movie} />}</Col>
      </Container>
...
//       </Container>
//       <Container className="justify-content-md-center">
//         {/* <Col>{loading ? <Spinner /> : comments}</Col> */}
//         {comments}
//       </Container>
